...
def forward_llama(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor],
    ...
    if self.training and self.gradient_checkpointing:
        ...
        + self.adapter_mlp(h_norm, vis_weight) * self.s 
        + self.adapter_mlp(h_norm, half_vis_weight) * self.s
    else:
...
def set_Llama_Adapter(model, model_type, s=1, gradient_checkpointing=False):
    for _ in model.children():
        if type(_) == model_type:
