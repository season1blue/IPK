...
class Transformer(nn.Module):
    def __init__(self, params: ModelArgs):
        ...
        self.adapter_imgatt = ImgAttention()
    ...
    def forward(self, examples, labels, images=None, half_images=None, prefix_img=None, prefix_nonimg=None, img_indicators=None):
        ...
        img_tx_prefix_emb = self.tok_embeddings(prefix_img).unsqueeze(0).expand(bsz, -1, -1) 
        ...
        image_embeds = torch.cat([compressed_img_embs, 
                                torch.zeros(compressed_img_embs.size(0), self.adapter_emb1.size(1) - compressed_img_embs.size(1), compressed_img_embs.size(2)).half().to(compressed_img_embs.device)],
                                 dim=1)

        half_image_embeds = self.adapter_proj(half_image_embeds) * 0.01
        half_image_embeds = half_image_embeds * img_indicators.half().view(-1, 1, 1)
        half_img_compress = aggregate_vit_features(torch.concat([torch.zeros(bsz, 1, half_image_embeds.shape[2], device='cuda', dtype=half_image_embeds.dtype), half_image_embeds], dim=1), level=3)
        half_img_compress = self.adapter_imgatt(half_img_compress, half_image_embeds)
        half_image_embeds = torch.cat([half_img_compress, 
                                  torch.zeros(half_img_compress.size(0), self.adapter_emb1.size(1) - half_img_compress.size(1),half_img_compress.size(2)).half().to(half_img_compress.device)],
                                 dim=1)
    ...
    def generate(
        ...
        compress_img_emb = aggregate_vit_features(torch.concat([torch.zeros(bsz, 1, image_embeds.shape[2], device='cuda', dtype=image_embeds.dtype), image_embeds], dim=1), level=3)
        compress_img_emb = self.adapter_imgatt(compress_img_emb, image_embeds)
        

        image_embeds = torch.cat(
            [compress_img_emb, 
             torch.zeros(
                 compress_img_emb.size(0), 
                 self.adapter_emb1.size(1) - compress_img_emb.size(1),
                 compress_img_emb.size(2)).half().to(image_embeds.device)
                 ], dim=1)

        half_image_embeds = half_image_embeds * indicators.half().view(-1, 1, 1)
        
        half_abs = aggregate_vit_features(torch.concat([torch.zeros(bsz, 1, half_image_embeds.shape[2], device='cuda', dtype=half_image_embeds.dtype), half_image_embeds], dim=1), level=3)

        half_compress = self.adapter_imgatt(half_abs, half_image_embeds)
        

        half_image_embeds = torch.cat([half_compress, torch.zeros(half_compress.size(0), self.adapter_emb1.size(1) - half_compress.size(1),
                                                            half_compress.size(2)).half().to(half_compress.device)], dim=1)
...
def aggregate_vit_features(features, level, patch_size=14, image_size=224): 
    ...
    num_patches_per_side_new = num_patches_per_side // downscale_factor
    ...
    aggregated_patches = torch.nn.functional.avg_pool2d(
        patch_tokens_2d.permute(0, 3, 1, 2),
